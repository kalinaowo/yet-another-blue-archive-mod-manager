...
def writeNewModName(modPath, newModName):
    mods_dict = modData.setdefault("mods", {})
    mods_dict[modPath] = {"name":newModName}
    saveDataFile()

def deleteModName(modPath):
    if "mods" in modData and modPath in modData["mods"] and "name" in modData["mods"][modPath]:
        del modData["mods"][modPath]["name"]
    saveDataFile()

def setModApplied(modPath, status):
    mods_dict = modData.setdefault("mods", {})
    mod_entry = mods_dict.setdefault(modPath, {})
    mod_entry["isApplied"] = status
    saveDataFile()

def getIsModApplied(modPath):
    if "mods" in modData and modPath in modData["mods"] and "isApplied" in modData["mods"][modPath]:
        return modData["mods"][modPath]["isApplied"]
    else:
        return False
...
def retrieveCharacterNameTranslations(char):
    char_path = char.modName
    orgChar = char_path
    save_path = "student_names.json"
